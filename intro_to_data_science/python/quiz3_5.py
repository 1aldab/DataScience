...
def compute_cost(features, values, theta):
    ...
    m = len(values)
    cost = ((np.dot(features, theta) - values) ** 2).sum()
    return cost / (2 * m)
...
predictions, plot = predictions(turnstile_df)
print pandas.DataFrame({'predicted': predictions, 'values': values})
print "The coefficient of determination (R-squared) is", compute_r_squared(values, predictions)
plot.show()
